var tour = new Tour({
    ...
    steps: [
	{
	    ...
	    content: "<p>We'll walk through the main features of the transcription simulator.  This is the main screen which summarises all the simulations.</p><p>The page should have loaded with our default settings - any changes you make to the setup will not be reflected in the animations <b>until you press the recalculate button</b> because the simulation itself can take several minutes to complete.</p>",
	    placement: "right"
	},
	{
	    element: "#tour-button",
	...
	},
	{
	    ...
	    content: "<p>Here we see a single simulation of the polymerases running along a specific gene. As the gene is so long, we have to wrap it starting from top left to bottom right.</p><p>Damage sites are highlighted in red, and repaired sites in green.  As time progresses, polymerases are introduced according to the initiation rate, and depending on the way the simulation has been set up, they encounter damage sites and start to queue up.</p><p>These stalled polymerases can be removed either by dissociation (in which case they are available for initiation again) or degraded - in either case, their exit is visualised by their falling to the bottom of the animation.</p><p>Moving polymerases are also subject to random removal from the gene body when processivity is not perfect. In the default settings, processivity is high (RNAPII stays on strand two hours on average.)</p>",
	    placement: "right"
	},
	{
